"""AeroVision LK — HuggingFace Spaces entry point.

Self-contained Gradio demo: YOLOv8s + SAHI aerial vehicle detection on
VisDrone imagery. Compare Standard YOLO (640px) vs SAHI (512px tiles).
"""
...
WEIGHTS_PATH = Path(__file__).parent / "yolov8s_int8.onnx"
...
yolo_model = YOLO(str(WEIGHTS_PATH), task="detect")

torch.cuda.is_available = _orig_cuda

print(f"Models loaded: {WEIGHTS_PATH.name}")
...
if EXAMPLES_DIR.exists():
    for p in sorted(EXAMPLES_DIR.glob("*.jpg")):
        examples.append([str(p), "SAHI (recommended)"])
